...
const testArticle = 'testArticle' + Date.now();
const editArticle = 'editArticle' + Date.now();
const testArticleTag = 'testTag' + Date.now();
const testArticleBarcode = 'testBarcode' + Date.now();
...
describe('article', () => {
  ...
  it('finds article by tag and deletes tag barcode and article', () => {
    ...
    cy.findByLabelText(en.ARTICLE_ADD_FORM_AMOUNT_LABEL)
      .clear()
      .type('800');
    cy.findByTitle(en.ARTICLE_ADD_FROM_ACCEPT).click();
